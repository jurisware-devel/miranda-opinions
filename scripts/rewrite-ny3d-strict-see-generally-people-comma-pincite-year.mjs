...
if (years.length === 0) {
  console.error('Usage: rewrite-ny3d-strict-see-generally-people-comma-pincite-year.mjs <year> [year...]');
  process.exit(1);
}
...
// Strict target:
// [#]see generally People v[.] <name>, <vol> NY3d <page>, <pincite> [<year>]
const strictRe = /\[(\d+)\]((?:see\s+generally)\s+People\s+v\.?\s+[^,\n]+,\s*\d+\s+NY3d\s+\d+,\s*\d+(?:\s*[-–—]\s*\d+)?\s*\[\d{4}\])/gi;
