...
if (years.length === 0) {
  console.error('Usage: rewrite-ny3d-hashyes-people-all.mjs <year> [year...]');
  process.exit(1);
}
...
const signal = '(?:see generally|see\\s+e\\.g\\.|see also|see|cf\\.?|but cf\\.?|but see|contra|accord|compare)';
const citeSuffix = '(?:,\\s*\\d+\\s+NY3d\\s+\\d+(?:,\\s*\\d+(?:\\s*[-–—]\\s*\\d+)?)?(?:\\s*\\[\\d{4}\\])?|\\(\\s*\\d+\\s+NY3d\\s+\\d+(?:,\\s*\\d+(?:\\s*[-–—]\\s*\\d+)?)?(?:\\s*\\[\\d{4}\\])?\\s*\\))';
const strictRe = new RegExp(
  String.raw`\[(\d+)\]\s*((?:${signal})\s+)?(People\s+v\.?\s+[^,\n()]{1,180}?${citeSuffix})`,
  'gi'
);
...
for (const file of files) {
  ...
  body = body.replace(strictRe, (full, refNum, sig, citation, offset, src) => {
    const ref = refs.get(refNum);
    if (!ref) return full;
    if (ref.caseId === currentId) return full;

    // Skip if this token is already inside an existing markdown link label.
    if (offset > 0 && src[offset - 1] === '[') {
      const after = src.slice(offset + full.length, offset + full.length + 2);
    ...
    return `${sig || ''}[${citation}](https://miranda.jurisware.com/case/${ref.caseId}/)`;
  });
