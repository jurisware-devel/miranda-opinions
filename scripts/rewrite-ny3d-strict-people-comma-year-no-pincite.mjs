...
if (years.length === 0) {
  console.error('Usage: rewrite-ny3d-strict-people-comma-year-no-pincite.mjs <year> [year...]');
  process.exit(1);
}
...
// Strict target:
// [#]People v[.] <name>, <vol> NY3d <page> [<year>]
const strictRe = /\[(\d+)\](People\s+v\.?\s+[^,\n]+,\s*\d+\s+NY3d\s+\d+\s*\[\d{4}\])/g;
...
for (const file of files) {
  ...
  const updatedBody = body.replace(strictRe, (full, refNum, citation) => {
    ...
    return `[${citation}](https://miranda.jurisware.com/case/${ref.caseId}/)`;
  });
...
console.log(JSON.stringify({ years, files_scanned: files.length, files_changed: filesChanged, rewrites, linked_case_ids_count: linkedCaseIds.size, linked_case_ids_sample: [...linkedCaseIds].slice(0, 20) }, null, 2));
